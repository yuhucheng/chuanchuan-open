# chuanchuan-open · 串串开源基础

串串的开源客户端基础工程：Flutter 界面、局域网设备发现、文件准备与平台适配。它是独立工程，可单独构建和复用；不需要官方邀请码或闭源仓库来运行本工程已实现的基础功能。

品牌名称为 `chuanchuan`，仓库名为 `chuanchuan-open`。Dart 包名 `share_hub_open`、`ShareHubApp` 类型和已有应用标识暂作兼容使用。
...
macOS Debug 构建与实机启动已通过，预览页明确显示媒体不可用；35 项 Flutter 测试、静态分析和 4 项 Swift 平台测试通过，详见 [macOS 验证](docs/validation/macos-foundation.md)。Windows 构建及测试结果见 [拆分验证](docs/validation/project-split.md)，本轮未复测。调试产物没有可信发行签名，Windows 安全策略可能阻止启动；复制程序时须包含同目录 DLL 和 `data`。
...
`ShareHubApp` 在同一组件生命周期内持有初次注入的平台、文件与媒体实例；更换整套依赖时使用新的组件 key。来源刷新后清空选择，必须由用户明确重新选择。旧会话回调隔离、首帧等待和清理失败重试由公共控制器编排，实际媒体资源由注入的引擎管理。
