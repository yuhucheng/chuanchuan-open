...
Future<void> probeRecovery(
  ...
  final left = ConnectionRecoveryService();
  final right = ConnectionRecoveryService();
  await left.open(address: InternetAddress.loopbackIPv4);
  await right.open(address: InternetAddress.loopbackIPv4);
  ...
  try {
    ...
    final originalGrant = original.grant;
    final remoteGrant = previousRemote.grant;
    final suspended = Future.wait([
      original.phaseChanges.firstWhere((p) => p == ConnectionPhase.suspended),
      previousRemote.phaseChanges.firstWhere(
        (p) => p == ConnectionPhase.suspended,
      ),
    ]);
    proxy.cut();
    await suspended.timeout(const Duration(seconds: 10));
    if (original.isClosed || previousRemote.isClosed) {
    ...
    await Future.wait([
      left.reconnectVia(original, (_) => original!.openRelayWire(ca)),
      right.acceptVia(previousRemote, (_) => previousRemote.openRelayWire(cb)),
    ]).timeout(const Duration(seconds: 15));
    if (!original.isConnected ||
        !previousRemote.isConnected ||
        !identical(original.grant, originalGrant) ||
        !identical(previousRemote.grant, remoteGrant) ||
        original.grant!.generation != generation ||
        original.grant!.expiresMicros != expiry) {
      throw StateError('Relay recovery replaced or renewed the grant');
    }
    final delivered = Completer<VerifiedSessionMessage>();
    previousRemote.attachReceiver(
      onRequest: delivered.complete,
      resolveSession: (_) => null,
      onSignal: (_) {},
    );
    await original.sendRequest(
      await original.createRequest(
