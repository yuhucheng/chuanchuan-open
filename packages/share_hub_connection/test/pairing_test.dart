...
void main() {
  ...
  PairingAttempt attempt() =>
      PairingAttempt(identity: clientIdentity, clock: () async => now);
  ...
  test('authorization remains valid after two hours and expires at eight', () {
    expect(connectionLifetime, const Duration(hours: 8));
    final lease = SessionLease(startedMicros: now);
    expect(lease.check(now + const Duration(hours: 2).inMicroseconds), isTrue);
    expect(
      lease.check(now + const Duration(hours: 8).inMicroseconds - 1),
      isTrue,
    );
    expect(lease.check(now + const Duration(hours: 8).inMicroseconds), isFalse);
  });
  ...
  test('cancelled connection cannot be restored by late socket', () async {
    ...
    final result = request.connect('127.0.0.1', host.port!, host.offer!.code);
    request.cancel();
