...
> **For agentic workers:** 实施时使用 superpowers:executing-plans，按工作包逐项执行现有 tasks.md。当前用户授权同步全部未归档 change 和制定计划；本轮只更新规划文件。遵守既有 main 规则，不自动建分支/worktree。
...
- 本仓 change 的实现明细仍以自己的 tasks.md 为准，不复制对仓实现，也不把本计划当作任务已完成的证据。当前 124 项实施任务均未完成。
...
- [ ] Windows 在专用生成窗口复现首帧失败；先依当前 SDK README 配置真实测试宿主及绝对 fixture 路径，核对测试依赖后运行 SDK 集成测试。不得套用历史私有客户端路径。
...
当前指定的下一开发入口是 **D1 桌面事实与根因收敛**；C0/A0 契约和版本决定可并行准备。后续按 S1→S4 推进，未排期项先完成排期决定。本文不指定日历工期，也不把计划同步当作开始执行这些工作包的授权。
