...
适用客户端 0.1.0-dev.1。公开实现为 `packages/share_hub_connection`，不依赖 UI 或媒体 SDK。当前支持 macOS 客户端接入及通用 Dart 协议；Windows 安全存储/入口、远端媒体、官方/内网辅助服务、双机验收未完成。该实现未经独立密码协议审计，不作为正式发行安全认证。
