...
公共包 `share_hub_media_api` MUST 独立提供 Flutter 媒体契约，不依赖 SDK 实现或客户端 UI。当前包 API 版本为 0.4.0，规格版本独立维护。
...
### Requirement: 远控公共契约不等同系统执行

公开 API MUST 分别表达控制请求能力、精确认证授权、几何/画面代次、输入/释放/停止消息及独立文本剪贴板代次。仅观看、投屏或文件操作的授权不得升级成控制授权；公开消息和纯状态对象不得自行声明系统输入、系统剪贴板或画面已就绪。

#### Scenario: 仅观看授权携带控制消息

- **WHEN** 仅观看操作的消息体伪装成控制开始请求或输入事件
- **THEN** 精确授权上下文拒绝创建控制操作，不执行系统输入。

#### Scenario: 文本与剪贴板边界

- **WHEN** 控制者提交 `text-input`，或同一控制对产生纯文本剪贴板更新
- **THEN** 两种消息使用各自的能力、大小和代次门槛；`text-input` 不读写 OS 剪贴板，剪贴板消息不能充当键盘输入。
...
2026-09-23：公共媒体 API 0.4.0 导出远控授权、消息 codec 与纯状态契约，仍不声明系统输入或剪贴板宿主已实现。媒体 SDK 的实际执行、客户端入口、Windows/macOS 和双机验收由活跃 `plan-remote-control` change 继续追踪；本基线保留 `delivery: partial`，不表示 v0.2.0 整体已交付。

## 文档版本

规格版本：0.4.0。这是当前行为与约束说明，具体实现和平台验收状态见[验证摘要](../validation.md)。公开接口源码见[媒体 API](../../packages/share_hub_media_api/README.md)。
